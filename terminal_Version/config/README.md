...
- **Path**: `terminal_Version/config/autogitpush.conf`
- **Template**: `terminal_Version/config/autogitpush.conf.template`
...
1. **First Run**: The script will automatically create `terminal_Version/config/autogitpush.conf` if it doesn't exist
2. **Edit Configuration**: Update the settings in the config file:
   ```bash
   # Edit the configuration
   nano terminal_Version/config/autogitpush.conf  # Linux/macOS
   notepad terminal_Version\config\autogitpush.conf  # Windows
   ```
3. **Run Script**: Execute the script after configuration
   ```bash
   ./terminal_Version/src/gitautoB.sh     # Linux/macOS
   .\terminal_Version\src\gitautoP.ps1    # Windows PowerShell
   ```
